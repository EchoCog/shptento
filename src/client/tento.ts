...
export class Tento<
    ...
    constructor(client: Client, metaobjectSchema: TMetaobjectSchema, metafieldSchema: TMetafieldSchema, prefix: string = 'tento') {
        this._ = { client, schema: { ...metaobjectSchema, ...metafieldSchema }, prefix };

        this.metaobjects = Object.fromEntries(
            Object.entries(metaobjectSchema)
                .map(([key, metaobject]) => [key, new ShopifyMetaobjectOperations(metaobject, client, prefix)]),
        ) as TentoMetaobjectOperationsMap<TMetaobjectSchema>;
        this.products = new ShopifyProductOperations(client, prefix);
    }
...
export function tento<TSchema extends Record<string, unknown>>(
    ...
    const { client: clientSource, schema: rawSchema, prefix } = config;
    const client = createClientFromSource(clientSource);
    const metaobjectSchema = Object.fromEntries(
        Object.entries(rawSchema).filter(
            (e): e is [(typeof e)[0], Metaobject<any>] => {
    ...
    const metafieldSchema = Object.fromEntries(
        Object.entries(rawSchema).filter(
            (e): e is [(typeof e)[0], Metafield] => {
                return e[1] instanceof Metafield;
            }),
    ) as ExtractMetafieldSchema<TSchema>;

    return new Tento(client, metaobjectSchema, metafieldSchema, prefix);
}