...
export class SelectBuilder<TSelection extends SelectedFields | undefined> {
		...
		return Object.entries(obj)
			.map(([key, value]) => {
				if (typeof value === 'object' && value instanceof Metafield) {
					...
					return `${namespace}_${key}: metafield(namespace: "${namespace}", key: "${key}") { id, key, description, namespace, ownerType, type, value }`;
				}
	...
	private mapItemResult(node: any): ResultItem<TSelection> {
