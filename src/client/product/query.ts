...
export class ShopifyProductOperations {
    ...
    list(fields?: SelectedFields): SelectBuilder<SelectedFields | undefined> {
        return new SelectBuilder({ fields: fields ?? undefined, client: this.client, prefix: this.prefix });
    }

    update(id: string): UpdateBuilder {
        return new UpdateBuilder({ productId: id, client: this.client, prefix: this.prefix });
    }
}
