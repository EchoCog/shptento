...
export type ResultItem<TFields extends Record<string, keyof InferSelectModel | Metafield> | undefined> =
	TFields extends undefined
		? Simplify<DefaultInferSelect>
		: Simplify<{
				[K in keyof TFields]: TFields[K] extends Metafield
					? Simplify<InferSelectModel['metafield']>
					: // @ts-expect-error
					  InferSelectModel[TFields[K]];
		  }>;

export type UpdateResultItem<TFields extends ListConfigFields<InferUpdateModel>> = Simplify<{
	// @ts-expect-error
